  ...
  def validate
    ...
    case state
      when ConvertState::RECEIVED
        ...
        errors.add(:received_file_path, I18n.t(:file_not_exist, scope:'errors')) unless File.exist?(received_file_path)
      when ConvertState::FINISHED
        validates_presence [:received_file_path, :converted_file_path]
        errors.add(:received_file_path, I18n.t(:file_not_exist, scope:'errors')) unless File.exist?(received_file_path)
        errors.add(:converted_file_path, I18n.t(:file_not_exist, scope:'errors')) unless File.exist?(converted_file_path)
    end
  end
end